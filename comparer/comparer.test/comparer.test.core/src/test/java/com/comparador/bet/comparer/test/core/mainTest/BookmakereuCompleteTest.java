...
public final class BookmakereuCompleteTest extends AbstractBookmakereuDecorator {
	...
	public void verificarLecturaXml() {
		...
		for (BeanLectura match : getEventosLeidos()) {
			...
			} else if (match.getXmlMatch().getName().equalsIgnoreCase("CHELSEA FC vs WEST HAM UNITED")) {
				assertTrue(match.getXmlMatch().getXmlMarkets().size() == Integer.valueOf(3));
				for (XmlMarket market : match.getXmlMatch().getXmlMarkets()) {
					 if (market.getXmlBetType().getBetType().equals(BetTypeBookmaker.MAS_MENOS)) {
						assertTrue(market.getXmlMarketBets().size() == 2);
						for (XmlMarketBet bet : market.getXmlMarketBets()) {
							XmlMoreLessAttribute atributo = (XmlMoreLessAttribute) bet.getXmlAttribute();
							if (atributo.getTotalGoal() == 2) {
					 ...
					 } else if (market.getXmlBetType().getBetType().equals(BetTypeBookmaker.GANADOR_PARTIDO)) {
						 assertTrue(market.getXmlMarketBets().size() == 2);
						 for (XmlMarketBet bet : market.getXmlMarketBets()) {
							 XmlMatchWinnerAttribute atributo = (XmlMatchWinnerAttribute) bet.getXmlAttribute();
							 if (atributo.getResult().equals(Result.ONE)) {
								 assertTrue(bet.getXmlMatchParticipant().getName().equals("CHELSEA FC"));
								 assertTrue(bet.getXmlMarketBetOdd().getOdds().equals("1.05"));
								 assertTrue(bet.getXmlMarketBetOdd().getAmericanOdds().equals("-2000"));
							 } else if (atributo.getResult().equals(Result.TWO)) {
								 assertTrue(bet.getXmlMatchParticipant().getName().equals("WEST HAM UNITED"));
								 assertTrue(bet.getXmlMarketBetOdd().getOdds().equals("3.30"));
								 assertTrue(bet.getXmlMarketBetOdd().getAmericanOdds().equals("230"));
							 } else {
								 fail("Error en la lectura del xml");
					...
					for (XmlMarket market : match.getXmlMatch().getXmlMarkets()) {
						if (market.getXmlBetType().getBetType().equals(BetTypeBookmaker.GANADOR)) {
							assertTrue(market.getXmlMarketBets().size() == 5);
							for (XmlMarketBet bet : market.getXmlMarketBets()) {							
								if (bet.getXmlMatchParticipant().getName().equals("MANCHESTER UNITED")) {
									assertTrue(bet.getXmlMarketBetOdd().getOdds().equals("2.85"));
									assertTrue(bet.getXmlMarketBetOdd().getAmericanOdds().equals("185"));
									...
									assertTrue(bet.getXmlMarketBetOdd().getOdds().equals("101.00"));
									assertTrue(bet.getXmlMarketBetOdd().getAmericanOdds().equals("10000"));
								} else if (bet.getXmlMatchParticipant().getName().equals("WEST HAM UNITED")) {
									assertTrue(bet.getXmlMarketBetOdd().getOdds().equals("126.00"));
									assertTrue(bet.getXmlMarketBetOdd().getAmericanOdds().equals("12500"));
								} else {
									fail("Error en la lectura del xml");
								}
							}
	...
	public void verificarXmlGuardadosBD() {
		...
		for (RtMatch match : getEventosProcesados()) {
			if (match.getName(null).equals("Manchester United vs Aston Villa")) {
			...
			} else if (match.getName(null).equals("Chelsea FC vs West Ham")) {
				assertTrue(match.getRtMarkets().size() == Integer.valueOf(3));
				for (RtMarket market : match.getRtMarkets()) {
					if (market.getBetType().getNameId().equals(BetTypeBookmaker.MAS_MENOS.getId())) {
						assertTrue(market.getBets().size() == 2);
						for (RtBet bet : market.getBets()) {
							RtMasMenosAttribute atributo = (RtMasMenosAttribute) bet.getAttribute();
							if (atributo.getTotalGoalValue() == 2) {
					...
					} else if (market.getBetType().getNameId().equals(BetTypeBookmaker.GANADOR_PARTIDO.getId())) {
						assertTrue(market.getBets().size() == 2);
						for (RtBet bet : market.getBets()) {
							...
							if (atributo.getResult().equals(Result.ONE)) {
								...
								assertTrue(bet.getBetOdd().getAmericanOdds().equals("-2000"));
				...
				for (RtMarket market : match.getRtMarkets()) {
					if (market.getBetType().getNameId().equals(BetTypeBookmaker.GANADOR.getId())) {
						assertTrue(market.getBets().size() == 5);
						for (RtBet bet : market.getBets()) {
							if (bet.getParticipant().getCfgParticipant().getName(null).equals("Manchester United")) {
								assertTrue(bet.getBetOdd().getOdds().equals("2.85"));
								assertTrue(bet.getBetOdd().getAmericanOdds().equals("185"));
								...
								assertTrue(bet.getBetOdd().getOdds().equals("101.00"));
								assertTrue(bet.getBetOdd().getAmericanOdds().equals("10000"));
							} else if (bet.getParticipant().getCfgParticipant().getName(null).equals("West Ham")) {
								assertTrue(bet.getBetOdd().getOdds().equals("126.00"));
								assertTrue(bet.getBetOdd().getAmericanOdds().equals("12500"));
							} else {
								fail("Error en la lectura del xml");
							}
						}
