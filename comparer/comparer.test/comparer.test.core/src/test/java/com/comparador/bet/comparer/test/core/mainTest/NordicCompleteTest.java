...
import com.comparador.bet.comparer.test.core.mainTest.decorators.AbstractNordicDecorator;
...
/**
 * The Class NordicCompleteTest.
 */
@ActiveProfiles({ ProfileConstant.TEST, ProfileConstant.TEST_NO_CREATE_DATA })
public final class NordicCompleteTest extends AbstractNordicDecorator {
		...
		for (BeanLectura match : getEventosLeidos()) {
			if (match.getXmlMatch().getName()
				...
				for (XmlMarket market : match.getXmlMatch().getXmlMarkets()) {
					...
					.getXmlMatch()
					.getName()
					.equals("United States of America NBA Season Bets Winner of NBA Season 2013/14 (Inc. Play-offs)")) {
				assertTrue(match.getXmlMatch().getXmlMarkets().size() == Integer
						.valueOf(1));
				assertEquals(
						"United States of America NBA Season Bets",
						match.getXmlMatch().getXmlTournament().getName());
				for (XmlMarket market : match.getXmlMatch().getXmlMarkets()) {
					if (market.getXmlBetType().getBetType()
							.equals(BetTypeNordicbet.GANADOR)) {
						assertTrue(market.getXmlMarketBets().size() == 30);
						assertTrue(match.getXmlMatch()
								.getXmlMatchParticipants().size() == 31);
		...
		for (RtMatch match : getEventosProcesados()) {
			if (match.getName(null).equals("Manchester United vs Aston Villa")) {
				...
				for (RtMarket market : match.getRtMarkets()) {
					...
					.equals("United States of America NBA Season Bets Winner of NBA Season 2013/14 (Inc. Play-offs)")) {
				assertTrue(match.getRtMarkets().size() == Integer.valueOf(1));
				assertEquals("NBA", match.getMatchId().getCompetition()
						.getName(null));
				for (RtMarket market : match.getRtMarkets()) {
					if (market.getBetType().getNameId()
							.equals(BetTypeNordicbet.GANADOR.getId())) {
						assertTrue(market.getBets().size() == 30);
						assertTrue(match.getMatchId().getParticipiants().size() == 31);
