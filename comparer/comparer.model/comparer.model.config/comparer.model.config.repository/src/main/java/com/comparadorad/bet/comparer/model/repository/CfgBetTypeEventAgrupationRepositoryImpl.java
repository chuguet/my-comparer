/**
 *
 * Copyright (C) FACTORIA ETSIA S.L.
 * All Rights Reserved.
 * www.factoriaetsia.com
 *
 */
package com.comparadorad.bet.comparer.model.repository;

import com.comparadorad.bet.comparer.model.config.bean.CfgBetTypeEventAgrupation;
import com.comparadorad.bet.comparer.model.core.repository.IGenericCustomRepository;

/**
 * The Class CfgBetTypeEventAgrupationRepositoryImpl.
 */
class CfgBetTypeEventAgrupationRepositoryImpl extends
		AbstractCfgRepository<CfgBetTypeEventAgrupation> implements
		IGenericCustomRepository<CfgBetTypeEventAgrupation> {

}
