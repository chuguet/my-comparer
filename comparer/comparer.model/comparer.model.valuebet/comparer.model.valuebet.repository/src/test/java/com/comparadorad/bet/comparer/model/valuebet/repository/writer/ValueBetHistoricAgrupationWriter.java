...
/**
 * The Class ValueBetHistoricAgrupationWriter.
 */
public class ValueBetHistoricAgrupationWriter extends
		AbstractWriterXML<List<ValueBetHistoricData>> {
	 ...
	 * @return the match
	 */
	private InfoMatch getMatch(String pId, Date date) {
	 ...
	 */
	@Override
	protected List<ValueBetHistoricData> makeObject() {
		List<ValueBetHistoricData> result;
		ValueBetHistoricData valueBetHistoricData;

		result = new ArrayList<ValueBetHistoricData>();

		valueBetHistoricData = new ValueBetHistoricData();
		valueBetHistoricData.setInfoMatch(getMatch("21229",
				new Date(new Date().getTime() - 86400000l)));
		valueBetHistoricData.setBet(getBet("2.5"));
		valueBetHistoricData.setExpectation(getExpectation("1.4"));
		valueBetHistoricData.setProbability(getProbability("3.2"));
		valueBetHistoricData.setObjectId(new BigInteger("1"));
		RtBetTypeEvent betTypeEvent = new RtBetTypeEvent();
		betTypeEvent.setObjectId(new BigInteger("1"));
		valueBetHistoricData.setBetTypeEvent(betTypeEvent);

		result.add(valueBetHistoricData);
