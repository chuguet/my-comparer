...
public class CuotaConverterUtil {
	...
	/** The valor cuota. */
	private static double valorCuota = 0.0;
	...
	/**
	 * Gets the american odds by a decimal odd
	 * 
	 * @param betOdd
	 *            the bet odd
	 * @return the american odds
	 */
	public static String americanToDecimalOdds(final String betAmericanOdd) {
		valorCuota = Double.valueOf(betAmericanOdd);
		String result = "";
		StringUtil stringUtil = new StringUtil();
		if (valorCuota > 0) {
			valorCuota = (valorCuota / 100) + 1;
		} else {
			valorCuota = (-100 / valorCuota) + 1;
		}

		result = FormatterUtil.formatBet(valorCuota, 2);
		result = stringUtil.replaceCharacter(result, ",", ".");
		return result;
	 ...
	 * @return the string
	 */
	public String decimalToFra(double valorCuota) {
		...
		while (last < LIMIT) {
			...
			if ((Math.abs(goodness) >= MAX_GOODNESS))
				break;
			if (error <= 1 && numerator > 9 && denominator > 9)
				break;
		}
