...
<!-- Quitamos la funcionalidad de tratamiento -->
<%-- 	<aui:select bean="<%= selContact %>" label="title[person]" listType="<%= ListTypeConstants.CONTACT_PREFIX %>" listTypeFieldName="prefixId" model="<%= Contact.class %>" name="prefixId" showEmptyOption="<%= true %>" /> --%>
	...
	<c:choose>
		...
		<c:otherwise>
			...
			%>
			<aui:input bean="<%= displayEmailAddressUser %>" model="<%= User.class %>" name="emailAddress" disabled="<%=isNotAdmin%>">
...
<!-- Quitamos la funcionalidad de Segundo nombre -->
...
<!-- Quitamos la funcionalidad de Sufijo -->
...
<!-- Se quita la funcionalidad de mostrar el ID del usuario -->
...
<!-- Se quita la funcionalidad de Titulo -->
