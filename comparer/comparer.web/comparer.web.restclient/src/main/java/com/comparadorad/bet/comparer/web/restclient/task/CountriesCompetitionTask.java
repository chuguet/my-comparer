...
public class CountriesCompetitionTask extends AbstractLevelTask {
	/** The Constant LOG. */
	...
	public CountriesCompetitionTask(
		...
		try {
			RestTemplateUtil.executorServiceSender
					.submit(new RequestSender(
							restClient,
							".results/ResultsModule/rest/countryLongTermController/getHead",
							requestEntity, writeURLs)).get();

			RestTemplateUtil.executorServiceSender
					.submit(new RequestSender(
							restClient,
							".results/ResultsModule/rest/countryEventsController/getCountryEventsCompetition",
							requestEntity, writeURLs)).get();
		} catch (Exception e) {
